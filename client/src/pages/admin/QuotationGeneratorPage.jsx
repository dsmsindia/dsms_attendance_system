...
import { getQuotations, saveQuotation, downloadQuotation } from "../../api/quotations";
import { Calculator, Plus, Download, Edit, Trash2, Save, Users, Building } from "lucide-react";
...
export default function QuotationGeneratorPage() {
  ...
  const emptyForm = {
    refNo: `DSMS/${new Date().getFullYear()}/001`,
    date: new Date().toLocaleDateString("en-IN", { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '.'),
    to: "THE MANAGEMENT",
    companyName: "",
    address: "",
    isGstApplied: true, 
    manpower: [{ category: "SECURITY GUARD", rate: 14000, persons: 1, dutyDays: "26 Days", dutyHours: "08 Hrs.", gstPercent: 18 }]
  };
  ...
  const addManpowerRow = () => {
    setFormData({
      ...formData,
      manpower: [...formData.manpower, { category: "", rate: 0, persons: 1, dutyDays: "26 Days", dutyHours: "08 Hrs.", gstPercent: 18 }]
    });
  };
    ...
    <div className="w-full h-full flex flex-col space-y-6 pb-6 min-h-0">
      ...
      {view === "list" && (
        <div className="flex-1 bg-white rounded-xl border shadow-sm overflow-hidden flex flex-col min-h-[300px]">
          {loading ? (
             <div className="p-10 text-center text-slate-500 font-medium">Loading quotations...</div>
          ) : quotations.length === 0 ? (
             <div className="p-10 text-center text-slate-500 font-medium">No quotations found. Create your first one above.</div>
          ) : (
            <div className="flex-1 overflow-auto w-full relative">
              <table className="w-full text-sm text-left whitespace-nowrap border-collapse">
                <thead className="bg-slate-50 border-b sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="px-6 py-4 font-bold text-slate-700">Client / Company</th>
                    <th className="px-6 py-4 font-bold text-slate-700">Reference Details</th>
                    <th className="px-6 py-4 font-bold text-slate-700 text-center">Personnel</th>
                    <th className="px-6 py-4 text-right font-bold text-slate-700">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {quotations.map(q => (
                    <tr key={q._id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                          <div className="font-bold text-slate-900 text-base">{q.companyName}</div>
                          <div className="text-xs text-slate-500 font-medium mt-0.5 max-w-[250px] truncate">{q.address}</div>
                      </td>
                      <td className="px-6 py-4">
                          <div className="font-semibold text-slate-700">{q.refNo}</div>
                          <div className="flex items-center gap-2 mt-1">
                             <span className="text-xs font-medium text-slate-500">{q.date}</span>
                             {q.isGstApplied === false 
                                ? <Badge variant="outline" className="bg-slate-100 text-slate-600 border-slate-200 shadow-none h-5 text-[10px]">Non-GST</Badge>
                                : <Badge variant="outline" className="bg-indigo-50 text-indigo-700 border-indigo-200 shadow-none h-5 text-[10px]">GST Configured</Badge>
                             }
                          </div>
                      </td>
                      <td className="px-6 py-4 font-bold text-emerald-700 text-center">
                          {q.manpower.reduce((acc, curr) => acc + curr.persons, 0)} Guards
                      </td>
                      <td className="px-6 py-4 text-right space-x-2">
                          <Button variant="outline" size="sm" onClick={() => handleEdit(q)} className="font-bold text-slate-700 border-slate-300">
                              <Edit className="w-4 h-4 mr-1.5" /> Edit
                          </Button>
                          <Button variant="default" size="sm" onClick={() => downloadQuotation(q._id, q.companyName)} className="bg-indigo-600 hover:bg-indigo-700 font-bold shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-8 flex flex-col">
            <div className="space-y-4 border-b pb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-1.5"><Label className="font-bold text-slate-700 text-xs uppercase tracking-wide">Ref No.</Label><Input required value={formData.refNo} onChange={e => setFormData({...formData, refNo: e.target.value})} className="font-mono bg-slate-50 font-bold" /></div>
                <div className="space-y-1.5"><Label className="font-bold text-slate-700 text-xs uppercase tracking-wide">Date</Label><Input required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="font-mono bg-slate-50 font-bold" /></div>
                <div className="space-y-1.5"><Label className="font-bold text-slate-700 text-xs uppercase tracking-wide">Addressed To</Label><Input required value={formData.to} onChange={e => setFormData({...formData, to: e.target.value})} placeholder="e.g. THE MANAGEMENT" className="font-bold bg-slate-50" /></div>
                <div className="space-y-1.5 lg:col-span-3"><Label className="font-bold text-slate-700 text-xs uppercase tracking-wide">Company Name</Label><Input required value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} className="font-black text-indigo-900 text-lg h-12" placeholder="Enter full company name..." /></div>
                <div className="space-y-1.5 md:col-span-2 lg:col-span-3"><Label className="font-bold text-slate-700 text-xs uppercase tracking-wide">Full Address</Label><Input required value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="font-medium bg-slate-50" placeholder="Enter physical location address..." /></div>
              </div>
            </div>
