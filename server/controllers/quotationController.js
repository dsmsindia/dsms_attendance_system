  ...
  try {
    ...
    if (_id) {
      quotation = await Quotation.findByIdAndUpdate(
        _id,
        { refNo, date, to, companyName, address, isGstApplied, manpower },
        { new: true },
      );
    } else {
...
async function downloadQuotationPdf(req, res) {
  try {
    ...
    doc.on("end", () => {
      ...
      const safeName = quote.companyName
        .replace(/[^a-z0-9]/gi, "_")
        .toLowerCase();
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="Quotation_${safeName}.pdf"`,
      );
      res.send(pdfBuffer);
    });
    ...
    doc.text(
      "We provide professional security personnel and services to various societies and corporate establishments all over West Bengal. Our terms and conditions are as follows:",
      { align: "justify" },
    );
    doc.moveDown(1);
    ...
    doc
      .font("Helvetica-Bold")
      .text("A. CHARTER OF RESPONSIBILITIES", { underline: true });
    doc.moveDown(0.8);
    ...
    const dutyTerms = quote.manpower.map(
      (item) =>
        `Our ${item.category.toLowerCase()} personnel will perform ${item.dutyHours} shift duties round the clock at your premises.`,
    );
    ...
    const allTerms = [...baseTerms, ...dutyTerms, ...remainingTerms];
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(12)
      .text("ANNEXURE 'II'", { align: "center", underline: true });
    doc.moveDown(0.5);
    ...
    headers.forEach((h, i) =>
      doc.text(h, colX[i], tableTop + 6, { width: colW[i], align: "center" }),
    );

    doc
      .moveTo(50, tableTop + 22)
      .lineTo(545, tableTop + 22)
      .stroke();
    ...
    quote.manpower.forEach((item) => {
      ...
      doc.text(item.category.toUpperCase(), colX[0] + 5, currY, {
        width: colW[0] - 10,
        align: "left",
      });
      doc.text(item.rate.toLocaleString("en-IN"), colX[1], currY, {
        width: colW[1],
        align: "center",
      });
      doc.text(item.persons.toString(), colX[2], currY, {
        width: colW[2],
        align: "center",
      });
      doc.text(item.dutyDays, colX[3], currY, {
        width: colW[3],
        align: "center",
      });
      doc.text(item.dutyHours, colX[4], currY, {
        width: colW[4],
        align: "center",
      });
      ...
      if (isGst) {
        ...
        doc.text(rowTotal.toLocaleString("en-IN"), colX[5], currY, {
          width: colW[5],
          align: "center",
        });
        doc.text(gstAmt.toLocaleString("en-IN"), colX[6], currY, {
          width: colW[6],
          align: "center",
        });
      } else {
    ...
    colX.forEach((x) => {
      doc.moveTo(x, tableTop).lineTo(x, currY).stroke();
    });
    ...
    doc.text(footerText, colX[0] + 5, currY + 6, {
      width: footerLabelW,
      align: "right",
    });
    doc.text(`Rs. ${grandTotal.toLocaleString("en-IN")}`, colX[5], currY + 6, {
      width: totalW,
      align: "center",
    });
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(11)
      .text("B. TERMS OF PAYMENT", 50, doc.y, { underline: true });
    doc.moveDown(0.5);
    doc.font("Helvetica").fontSize(10);
    doc.text(
      "1. Our bill will be submitted on the first day of the following month. Payment by cheque or bank transfer in settlement thereof is expected to be received by us within seven days. Cheques should be drawn in favor of 'DYNAMIC SECURITY AND MANPOWER SERVICES PVT. LTD.' and payable at any bank in West Bengal.",
      { align: "justify" },
    );
    doc.moveDown(0.5);
    doc.text(
      "2. In case of default or delay in payment, interest at the rate of 18% per annum will be charged.",
      { align: "justify" },
    );
    doc.moveDown(1.5);
    ...
    if (stampPath) {
      ...
      doc
        .font("Helvetica-Bold")
        .text(
          "For DYNAMIC SECURITY AND MANPOWER SERVICES PVT. LTD.",
          250,
          doc.y,
          { align: "right", width: 295 },
        );
      doc.moveDown(3);
      doc.text("Authorized Signatory", 300, doc.y, {
        align: "right",
        width: 245,
      });
    }
