    ...
    if (g.active) {
      if (!projectIdFilter || projectIdFilter === "all") {
        ...
        const currentMatch =
          ...
          g.projectId?.toString() === projectIdFilter;
...
async function downloadExcelReport(req, res) {
  ...
  [4, 5].forEach((rowNum) => {
    ...
    sheet.getRow(rowNum).eachCell({ includeEmpty: true }, (cell, colNumber) => {
      cell.fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: headerColors[colNumber - 1] || "FF1E2B3E" },
      };
      cell.alignment = {
        horizontal: "center",
        vertical: "middle",
  ...
  for (const guard of guards) {
    ...
    if (guardForReport.projectHistory && guardForReport.projectHistory.length) {
      historyIds = guardForReport.projectHistory
      ...
      const isAssignedToContext =
        ...
        (guard.projectId?.name === contextProjectName ||
          (guard.projectHistory || []).some(
            (h) =>
              h.endDate === null && h.projectId?.name === contextProjectName,
          ));
    ...
    if (pNames.length === 0) {
      const projectObj = projectsDocs.find((p) => p.name === assignedProjName);
      ...
      const assignedProjType = guard.isReliever
        ? ""
        : projectsDocs.find((p) => p.name === assignedProjName)?.type ||
          "WEEKLY OFF";
      ...
      localSplits.push({
        ...
        stats: {
          ...localStats,
          weekOffs: payableWeekOff,
          totalWorkingDays: pureDays + payableWeekOff,
        },
      });
    } else {
      for (const pName of pNames) {
        if (contextProjectName && contextProjectName !== pName) continue;
        const projectObj = projectsDocs.find((p) => p.name === pName);
        ...
        const pType =
          ...
          projectsDocs.find((p) => p.name === pName)?.type ||
          (guard.isReliever ? "" : "WEEKLY OFF");
  ...
  for (const data of allRowsToExport) {
    ...
    // FIX: Force blank tag for relievers
    let typeStr = "";
    if (!guard.isReliever) {
      const pType =
        split.projectType ||
        split.days.find((d) => d.projectName === split.projectName)
          ?.projectType ||
        "";
      typeStr =
        String(pType).toUpperCase() === "ALL DAY"
          ? "All Day"
          : String(pType).toUpperCase() === "WEEKLY OFF"
            ? "Weekly Off"
            : "";
    }
    const projNameWithDesc = split.projectName
      ? `${split.projectName}${typeStr ? ` (${typeStr})` : ""}`
      : "Unassigned";
    ...
    const statCols = [
      { col: DAY_COL_START + daysInRange, key: "P" },
      { col: DAY_COL_START + daysInRange + 1, key: "A" },
      { col: DAY_COL_START + daysInRange + 2, key: "DD" },
      { col: DAY_COL_START + daysInRange + 3, key: "HD" },
      { col: DAY_COL_START + daysInRange + 4, key: "OFF" },
      { col: DAY_COL_START + daysInRange + 5, key: "H" },
    ];
    ...
    tdCell.value = {
      formula: formulaStr,
      result: Number(split.stats.totalWorkingDays || 0),
    };
      ...
      if (d.status) {
          ...
          const richTextParts = [
            {
              text: d.status,
              font: {
                color: { argb: fontFor[d.status] || "FF000000" },
                bold: true,
              },
            },
        ...
        if (fillFor[d.status]) {
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: fillFor[d.status] },
          };
          if (!d.markedByAdmin)
            cell.font = { color: { argb: fontFor[d.status] }, bold: true };
        }
      } else {
        cell.value = "";
      }
  ...
  for (let i = 0; i < daysInRange; i++)
    sheet.getColumn(DAY_COL_START + i).width = 9;
